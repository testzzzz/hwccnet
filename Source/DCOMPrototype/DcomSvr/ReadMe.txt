...
应用程序向导已为您创建了此 DcomSvr 项目，作为编写服务 (EXE) 的起点。
...
DcomSvr.idl
    ...
    此文件将由 MIDL 编译器进行处理以生成：
        C++ 接口定义和 GUID 声明 (DcomSvr.h)
        GUID 定义                                (DcomSvr_i.c)
        类型库                                  (DcomSvr.tlb)
        封送处理代码                                 （DcomSvr_p.c 和 dlldata.c）

DcomSvr.h
    此文件包含 DcomSvr.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 DcomSvr.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
DcomSvrps.vcproj
	...
	dlldata.c、DcomSvr_i.c 和 DcomSvr_p.c，这些文件是
