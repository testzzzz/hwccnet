========================================================================
    活动模板库 : RSSPlugin 项目概述
========================================================================

应用程序向导已为您创建了此 RSSPlugin 项目，作为编写动态链接库 (DLL) 的起点。
...
RSSPlugin.idl
    ...
    此文件将由 MIDL 编译器进行处理以生成：
        C++ 接口定义和 GUID 声明 (RSSPlugin.h)
        GUID 定义                                (RSSPlugin_i.c)
        类型库                                  (RSSPlugin.tlb)
        封送处理代码                                 （RSSPlugin_p.c 和 dlldata.c）

RSSPlugin.h
    此文件包含 RSSPlugin.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 RSSPlugin.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
RSSPluginps.vcproj
	...
	dlldata.c、RSSPlugin_i.c 和 RSSPlugin_p.c，这些文件是
