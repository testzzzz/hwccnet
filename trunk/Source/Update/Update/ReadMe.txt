...
应用程序向导已为您创建了此 Update 项目，作为编写动态链接库 (DLL) 的起点。
...
Update.idl
    ...
    此文件将由 MIDL 编译器进行处理以生成：
        C++ 接口定义和 GUID 声明 (Update.h)
        GUID 定义                                (Update_i.c)
        类型库                                  (Update.tlb)
        封送处理代码                                 （Update_p.c 和 dlldata.c）

Update.h
    此文件包含 Update.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
Updateps.vcproj
	...
	dlldata.c、Update_i.c 和 Update_p.c，这些文件是
